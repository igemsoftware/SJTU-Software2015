...
for($i = 0;$i <= $#part; $i = $i + 1){
	if($desc{$part[$i]} == 1){
		my $sth_3 = $dbh->prepare("(SELECT Bri_id FROM brick where Des like '%".$function[$i]."%' order by Score)");
		$sth_3->execute();
...
for($i = 0;$i <= $#part; $i = $i + 1){
	if($desc{$part[$i]} == 0){
		$j = 0;
		my $sth_4 = $dbh->prepare("(SELECT Bri_id FROM brick where Btype like '%".$part_desc{$part[$i]}."%')");
		$sth_4->execute();
...
sub gscore{
	...
	for($i = 0;$i <= $#part; $i = $i + 1){
		if($desc{$part[$i]} == 1){
			for($j = 0;$j <= $#part; $j = $j + 1){
				if($j != $i){	
					my $sth_2 = $dbh->prepare("(SELECT S.Com_id,R.Score FROM (contain as S join contain as T on S.Com_id = T.Com_id) join combine as R on S.Com_id = R.Com_id where (S.Bri_id like '%".$part[$i]."%') and (T.Bri_id like '%".$part[$j]."%'))");
					$sth_2->execute();
